...
if (is.null(opt$f)) {
  if (interactive()) {
    opt$f <- 'stereo-texture-analysis/rgb_geotiff_L1_sample.tif'
  } else {
...
tmp <- rgb2hsv(r, g, b) %>% 
  t %>% 
  as.data.frame %>% 
  mutate(dgci = (((h - 1/6) / 1/6) + (1 - s) + (1 - v))/3)
dgci <- Image(tmp$dgci, dim = dim(i)[1:2], colormode = 'gray')
...
df <- data.frame(d = as.vector(dgci), 
                 e = as.vector(edge), 
                 roi = as.vector(roi)) %>%
  filter(roi > 0) %>%
  group_by(roi) %>%
...
if (opt$t == T) {
    write.table(df, file = opt$outputtable, sep = ";", dec = ".", row.names = FALSE, quote = FALSE)
}
